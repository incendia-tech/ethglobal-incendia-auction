...
export function convertG2Format(snarkjsG2) {
  return [
...
export function convertProofFormat(proofData) {
  return {
...
export function loadAndConvertProof(proofPath, publicPath) {
...
export function validateProofFormat(proofData) {
  try {
    // Check if proof has required fields
    if (!proofData.pi_a || !proofData.pi_b || !proofData.pi_c) return false;

    // Check G1 format (should have 2 elements)
    if (proofData.pi_a.length < 2 || proofData.pi_c.length < 2) return false;

    // Check G2 format (should be 2x2 array)
    if (
      proofData.pi_b.length < 2 ||
      proofData.pi_b[0].length < 2 ||
      proofData.pi_b[1].length < 2
    ) {
      return false;
    }
